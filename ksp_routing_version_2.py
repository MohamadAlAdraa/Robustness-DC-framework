            ...
            if max(temp_path_edges_util) <= 1.0:
                for edge_name in zip(path, path[1:]):
                    ...
                    temp_edge_utilization = float("{:.2f}".format(
                        float("{:.2f}".format(utilization.get(edge_name, 0))) + float("{:.2f}".format(alpha))))
...
G = read_graph_from_file(strat_path + 'StratAL256_8.txt')
# G1 = read_graph_from_file(xpander_path + 'XpanderAL2048_8.txt')
G2 = read_graph_from_file(jellyfish_path + 'JellyfishAL256_8.txt')

write_paths_to_file(G, 10000, 'C:/Users/umroot/PycharmProjects/datacenter/computed_k_paths/strat32_15_10000_v2')
write_paths_to_file(G2, 10000, 'C:/Users/umroot/PycharmProjects/datacenter/computed_k_paths/jellyfish32_15_10000_v2')


strat_paths = read_paths_from_file('C:/Users/umroot/PycharmProjects/datacenter/computed_k_paths/strat32_15_10000_v2.json')
# xpander_paths = read_paths_from_file('xpander20.json')
jellyfish_paths = read_paths_from_file('C:/Users/umroot/PycharmProjects/datacenter/computed_k_paths/jellyfish32_15_10000_v2.json')
...
for k in ks:
    ...
    for t in toposs:
        for j in number_of_servers_under_each_switch:
            alpha, num_of_b_c, num_of_ac_c, accepted_connections, blocked_connections, traffic_matrix, paths_of_accepted_connections, utilization, connections_on_edges = find_ksp_throughput(topos[t], k, all_to_all(topos[t], j), alpha_f, util, fp=paths_[t])
