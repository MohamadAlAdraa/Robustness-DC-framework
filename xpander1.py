...
class Xpander:
    ...
    def __init__(self, number_of_switches, number_of_hosts, number_of_hosts_per_switch, d_regular, spectral_gap='a', k_lift=2, liftIter=1):
        ...
        ################################################################################################################
        Initial_G, number_of_lifts = self.graph_initialization()
        # number_of_lifts = self.liftIter
        self.Initial_G = Initial_G
        # Apply the deterministic algorithm after each lifted graph
        print('number of lifts', number_of_lifts)
    ...
    def graph_initialization(self):
        number_of_lifts = int(math.ceil(math.log(self.number_of_switches/(self.d_regular+1), self.k_lift)))
        Initial_G = nx.random_regular_graph(self.d_regular, self.d_regular+1)
        return Initial_G, number_of_lifts
    ...
    def incremental_expansion_algo(self, G, number_of_switches):
        G1 = G.copy()
        N = number_of_switches - (self.d_regular+1)
        d = self.d_regular
        T = len(list(G1.nodes()))
